...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "gov-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GOV_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GOV_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gov-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/gov-pp-cli/feedback.jsonl.
When ` + "`GOV_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GOV_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gov-pp-cli feedback list
  gov-pp-cli feedback list --limit 5
  gov-pp-cli feedback list --json`,
