...
In local mode: searches locally synced data only.`,
  ...
  gov-pp-cli search "error timeout"

  # Force local search only
  gov-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  gov-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  gov-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'gov-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/gov-pp-cli/data.db)")
