...
type Config struct {
	...
	GovBoardKey   string            `toml:"board_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://137-184-151-136.sslip.io",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("GOV_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "gov-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("GOV_BOARD_KEY"); v != "" {
		cfg.GovBoardKey = v
		cfg.markEnvOverride("GovBoardKey")
		cfg.AuthSource = "env:GOV_BOARD_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.GovBoardKey != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) AuthHeader() string {
	...
	if c.GovBoardKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GOV_BOARD_KEY"
		}
		return "Bearer " + c.GovBoardKey
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.GovBoardKey = ""
	delete(c.envOverrides, "GovBoardKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GovBoardKey"] {
			out.GovBoardKey = c.fileConfig.GovBoardKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GovBoardKey":
		c.fileConfig.GovBoardKey = c.GovBoardKey
	}
}
