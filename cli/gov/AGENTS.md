# Gov Printed CLI Agent Guide

This directory is a generated `gov-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
gov-pp-cli doctor --json
gov-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
gov-pp-cli which "<capability>" --json
gov-pp-cli <command> --help
```
...
```bash
gov-pp-cli <command> --help
gov-pp-cli <command> --dry-run --agent
```
